...
logging.basicConfig(filename="overhead_squat.log",
                    format='%(asctime)s %(message)s',
                    filemode='w')


class overhead_squat:
    ...
    def check_down_position(self, hip_y_position, knee_y_position, elbow_position, shoulder_position):
        
        if hip_y_position > knee_y_position or check_close3(hip_y_position, knee_y_position):
            if elbow_position[1] < shoulder_position[1] and check_close3(elbow_position[0], shoulder_position[0]):
    ...
    def check_up_position(self, shoulder_position, hip_x_position, knee_x_position, elbow_position):
    ...
    def check_if_still_going_up(self, hip_y_position, iteration):
        ...
        # check next 5 frames
        for i in range(0, 5):
            #print((iteration + i + 1), (self.json_reader.number_of_files - 1))
            if (iteration + i + 1) >= self.json_reader.number_of_files:
                return False
            else:
                point, trust = self.json_reader.get_values(iteration + i + 1, (HIP_VALUE,))
    ...
    def get_knee_value(self, iteration):
        ...
        return knee_position_right if self.body_orientation == "RIGHT" else knee_position_left
    ...
    def get_elbow_value(self, iteration):
        while True:
            ...
            return elbow_position_right if self.body_orientation == "RIGHT" else elbow_position_left
    ...
    def get_bigtoe_value(self, iteration):
        while True:
            ...
            return bigtoe_position_right if bigtoe_position_right != 0 else bigtoe_position_left
    ...
    def check_exercise(self):
        ...
        for i in range(0, self.json_reader.number_of_files):
            ...
            heel_x_position = self.get_heel_value(i)[0]
            bigtoe_x_position = self.get_bigtoe_value(i)[0]
            self.check_body_orientation(bigtoe_x_position, heel_x_position)
            ...
            if not going_down and last_value_y > new_value_y:
                if first_rep_detected is True and new_value_y < first_rep_y_value + 50:
                    ...
                    if not self.check_down_position(last_value_y, knee_y_position, elbow_position, shoulder_position):
                ...
                if not self.check_if_still_going_down(new_value_y, i):
                    ...
                    if self.check_up_position(shoulder_position, last_value_x, knee_x_position, elbow_position) and not was_no_rep:
...
def main():
    squat = overhead_squat(3, "overhead_squat/output_json/")

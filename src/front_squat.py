...
class front_squat:
    ...
    def check_down_position(self, hip_y_position, knee_y_position, shoulder_y_position, elbow_y_position, wrist_x_position, shoulder_x_position):
        if hip_y_position < knee_y_position and check_close(shoulder_y_position, elbow_y_position) and check_close(shoulder_x_position, wrist_x_position):
    ...
    def check_up_position(self, shoulder_x_position, hip_x_position, knee_x_position, shoulder_y_position, elbow_y_position, wrist_x_position):
        if check_close(shoulder_x_position, hip_x_position) and check_close(hip_x_position, knee_x_position) \
                and check_close(shoulder_y_position, elbow_y_position) and check_close(shoulder_x_position, wrist_x_position):
        ...
        for i in range(0, self.json_reader.number_of_files + 1):
            ...
            if going_down and last_value_y < new_value_y and not check_close(new_value_y, last_value_y):
                if not self.check_if_still_going_down(new_value_y, i):
                    ...
                    self.counted_reps += 1
                    if self.check_down_position(last_value_y, knee_y_position, shoulder_position[1], elbow_y_position, wrist_x_position, shoulder_position[0]):
                        self.correct_reps += 1
                    else:
                        self.no_reps += 1
                ...
                if not self.check_if_still_going_up(new_value_y, i):
                    ...
                    wrist_x_position = self.get_wrist_value(i)[0]
                    self.counted_reps += 1
                    if self.check_up_position(shoulder_position[0], last_value_x, knee_x_position, shoulder_position[1], elbow_y_position, wrist_x_position):
                        self.correct_reps += 1
                    else:
                        self.no_reps += 1
